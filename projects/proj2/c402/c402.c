...
**/
	...
	while (true) {
		if (*tmp == NULL) {	// Dosel jsem k mistu, kde ma vzniknout novy uzel
		...
		// Pokud je klic mensi nez v aktualnim uzlu, pokracuju v novem kole cyklu s levym synem
		else if (Content < (*tmp)->Cont) tmp = &(*tmp)->LPtr;
		// Pokud je klic vetsi nez v aktualnim uzlu, pokracuju v novem kole cyklu s pravym synem
		else if (Content > (*tmp)->Cont) tmp = &(*tmp)->RPtr;
		else return;	// V pripade, ze uzel jiz existuje
	}
...
**/
	...
	while (ptr != NULL) {	// Prochazim leve uzly leveho podstromu dokud muzu, kazdy pritom vypisu a ulozim na stack
		BTWorkOut(ptr);
		SPushP(Stack, ptr);
		ptr = ptr->LPtr;
	}
...
**/
	...
	while(!SEmptyP(&stack)) {	// Dokud ve stacku neco je, neustale prochazim leve uzly levych podstromu pravych uzlu
...
**/
	...
	while (ptr != NULL) {	// Prochazim leve uzly leveho podstromu dokud muzu, kazdy pritom ulozim na stack
		SPushP(Stack, ptr);
		ptr = ptr->LPtr;
	}
...
**/
	...
	while(!SEmptyP(&stack)) {	// Dokud ve stacku neco je, neustale prochazim leve uzly levych podstromu pravych uzlu
		RootPtr = STopPopP(&stack);
		BTWorkOut(RootPtr);	// Rozdil je jen v tom, ze prvek vypisu az zde
...
**/
	...
	/*
	 * Prochazim leve uzly leveho podstromu dokud muzu, kazdy pritom ulozim na Pstack a zaznamena prvni pruchod na
	 * Bstack
	 */
	while (ptr != NULL) {
		SPushP(StackP, ptr);
...
**/
	...
	while(!SEmptyP(&stackP)) {	// Dokud ve stacku neco je, neustale prochazim leve uzly levych podstromu pravych uzlu
		RootPtr = STopPopP(&stackP);
		tmpBool = STopPopB(&stackB);
		if (tmpBool) {	// Projde se jeste jednou zprava
			SPushB(&stackB, false);
...
**/
	...
	while(!SEmptyP(&stack)) {	// Pouziti inorderu pro uvolneni uzlu
		tmp = STopPopP(&stack);
		Leftmost_Inorder(tmp->RPtr, &stack);
		free(tmp);
	}
	*RootPtr = NULL;	// Nakonec jeste uvedeni do stavu po inicializaci
}
